...
class MPU6050TapSensor : public binary_sensor::BinarySensor, public Component {
 public:
  ...
  void set_double_tap_delay(uint16_t delay) { this->double_tap_delay_ = delay; }  // New setter
 ...
 protected:
  ...
  uint8_t duration_;
  uint16_t double_tap_delay_;           // Configurable double-tap delay in milliseconds
  ...
  bool double_tap_in_progress = false;  // Tracks if a double-tap sequence is active
};
